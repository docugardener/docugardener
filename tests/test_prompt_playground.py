...
async def test_prompt_playground():
    ...
    try:
        ...
        pirate_prompt = "You are a pirate documentation assistant. Use 'Arrr' and 'Matey' in every response."
        config = PromptConfig(
            id=str(uuid.uuid4()),
            tenantId=tenant_id,
        ...
        agent.generator.generate = AsyncMock(return_value=type('obj', (object,), {'content': 'Arrr, here be your docs.'}))
        ...
        if "strict documentation verification agent" in second_call_sys_prompt:
             print(f"✅ Success: Second call used default Verifier prompt")
        else:
             print(f"❌ Error: Second call expected default verifier prompt, but got:\n{second_call_sys_prompt}")
