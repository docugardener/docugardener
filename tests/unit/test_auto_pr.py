...
async def test_process_fix_pr_success(db_session):
    ...
    tenant = Tenant(id="tenant-123", githubOrgId="12345", name="Test Org", appId="111", privateKey="encrypted_key")
    db_session.add(tenant)
    db_session.commit()
    
    repo = Repository(id="repo-456", tenantId=tenant.id, githubRepoId="67890", name="docugardener-test")
    db_session.add(repo)
    db_session.commit()
    ...
    with patch("src.pipeline.job_manager.get_db") as mock_get_db, \
        ...
        mock_committer_instance.create_pr.return_value = "https://github.com/TestOrg/docugardener-test/pull/43"
        ...
        MockGitCommitter.assert_called_once_with(
            installation_token="mock_github_token",
            owner="TestOrg",
            repo="docugardener-test"
        )
...
async def test_process_fix_pr_tenant_missing_org(db_session):
    ...
    job = Job(id="job-bad", tenantId=tenant.id, repositoryId=repo.id, prNumber=1, status=JobStatus.QUEUED, result={})
    db_session.add(job)
    db_session.commit()

    with patch("src.pipeline.handler.job_manager") as mock_jm, \
...
async def test_process_fix_pr_committer_fails(db_session):
    tenant = Tenant(id="tenant-fail", githubOrgId="123", name="Org", appId="111", privateKey="enc-key")
    db_session.add(tenant)
    db_session.commit()
