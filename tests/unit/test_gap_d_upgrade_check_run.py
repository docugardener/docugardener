...
class TestPostQuotaExceededCheckRun:
    ...
    async def test_creates_neutral_check_run(self):
        ...
        assert "quota" in call_kwargs["output"]["summary"].lower() or \
               "limit" in call_kwargs["output"]["title"].lower()
...
class TestPrQuota402WithCheckRun:
    ...
    async def test_pr_quota_exceeded_posts_check_run_before_402(self):
        ...
        with patch("src.api.webhooks._post_quota_exceeded_check_run", new_callable=AsyncMock) as mock_check_run, \
             ...
             patch("src.api.webhooks.SessionLocal", return_value=mock_session) if False else \
