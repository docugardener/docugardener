...
class TenantContext:
    def __init__(self, tenant_id: str, app_id: str, private_key: str, llm_config: dict | None = None, notification_config: dict | None = None, workflow_config: dict | None = None, plan: str = "FREE", installation_id: str | None = None):
...
def get_tenant_context(github_installation_id: str) -> TenantContext:
    ...
    try:
                ...
                for provider, encrypted_key in raw_config["keys"].items():
                    if encrypted_key:
                        try:
                            ...
                            decrypted_keys[provider] = encrypted_key  # pass through if not encrypted
        ...
        notification_config = tenant.notificationConfig if hasattr(tenant, 'notificationConfig') else None
