                ...
                "reasons": [
                    {
                        ...
                        else getattr(r, "reason", "Unknown"),
                        "entity": r.get("entity_name")
                        if isinstance(r, dict)
                        else getattr(r, "entity_name", "Unknown"),
                    }
                    for r in (
                        result.drift_analysis.required_updates if result.drift_analysis else []
                    )
                ],
                "items": [
                    {
                        ...
                        else getattr(item, "file_path", ""),
                        "severity": item.get("severity", "low")
                        if isinstance(item, dict)
                        else getattr(item, "severity", "low"),
                        "current_content": item.get("current_content", "")
                        if isinstance(item, dict)
                        else getattr(item, "current_content", ""),
                        "semantic_change": item.get("semantic_change", "")
                        if isinstance(item, dict)
                        else getattr(item, "semantic_change", ""),
                        "reasoning": item.get("reasoning", "")
                        if isinstance(item, dict)
                        else getattr(item, "reasoning", ""),
                    }
                    for item in (
                        result.drift_analysis.items
                        if result.drift_analysis and hasattr(result.drift_analysis, "items")
                        else []
                    )
                ],
            },
...
def ignore_drift_job(job_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            "Check Run updated to neutral/acknowledged", job_id=job_id, check_run_id=check_run_id
        )
            ...
            try:
                ...
                if tenant and tenant.workflowConfig:
                    ...
                    dispatcher = NotificationDispatcher(
                        ...
                        github_private_key=decrypt_credential(tenant.privateKey)
                ...
                logger.warning(
                    "Failed to post Jira ignore comment", ticket=jira_ticket_key, error=str(e)
                )
