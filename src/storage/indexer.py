...
class DocumentIndexer:
    ...
    def _discover_code_files(self, repo_path: Path) -> list[Path]:
        ...
        excluded = {"node_modules", ".git", "venv", ".venv", "__pycache__", "dist", "build", "test", "tests"}
        code_files = [
    ...
    async def _index_doc_file(
        ...
        for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
            doc_id = generate_record_id(relative_path, chunk.get("title", f"section_{i}"), "documentation")
            ...
            records.append(DocumentRecord.from_numpy(
                id=doc_id,
                vector=embedding,
