...
class CodeEntity:
    ...
    def is_public(self) -> bool:
        ...
        if self.name.startswith("_") and not (
...
class CodeParser:
    ...
    def _init_languages(self) -> None:
        ...
        try:
            ...
            self._languages[SupportedLanguage.JAVASCRIPT] = Language(
        ...
        try:
            ...
            self._languages[SupportedLanguage.TYPESCRIPT] = Language(
                tree_sitter_typescript.language_typescript()
            )
            logger.debug("Loaded tree-sitter-typescript")
        except (ImportError, AttributeError):
            logger.warning(
                "tree-sitter-typescript not installed or incompatible, falling back to regex"
            )
    ...
    def parse_content(
        self, content: str, file_path: str, language: SupportedLanguage
    ) -> list[CodeEntity]:
        """
        ...
        if parser:
            try:
                ...
                logger.warning(
                    "Tree-sitter parsing failed, falling back to regex",
                    file=file_path,
                    error=str(e),
                )
    ...
    def _parse_with_tree_sitter(
        self, content: str, file_path: str, language: SupportedLanguage, parser: Parser
    ) -> list[CodeEntity]:
    ...
    def _extract_python_ast_docstring(self, node: Any, content_bytes: bytes) -> str | None:
        ...
        if first_child.type == "expression_statement":
            ...
            if string_node.type == "string":
                doc = content_bytes[string_node.start_byte : string_node.end_byte].decode(
    ...
    def _parse_js_simple(self, content: str, file_path: str, lines: list[str]) -> list[CodeEntity]:
        """
