...
class EphemeralClone:
    ...
    def _get_authenticated_url(self) -> str:
        ...
        if self.clone_url.startswith("https://"):
            return self.clone_url.replace(
                "https://", f"https://x-access-token:{self.access_token}@"
            )
        return self.clone_url
    ...
    async def clone(self) -> Path:
        ...
        self._clone_path = Path(tempfile.mkdtemp(prefix="docugardener_", dir=settings.tmpfs_path))
        ...
        try:
            ...
            logger.info("Fetching from remote", ref=fetch_ref or "default branch", depth=self.depth)
