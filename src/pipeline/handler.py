...
from src.pipeline.analyzer import FileChange, PRAnalyzer, PRAnalysisResult
...
from src.pipeline.job_manager import job_manager, JobStatus
...
async def process_pull_request(
    ...
    if policy_rules:
        ...
        policy_blocks_merge = any(
            v.enforcement in ("blocking", "blocking-with-reason")
    ...
    try:
        ...
        if gh_repo is None:
            try:
                from src.github.app import get_github_client
                _id_client = get_github_client(installation_id, app_id=_app_id, private_key=_private_key)
                gh_repo = _id_client.get_repo(f"{owner}/{repo}")
            except Exception as _e:
                logger.warning("Could not resolve GitHub repo ID; using fallback '0'", error=str(_e))
    ...
    try:
        ...
        if job_id:
            if result.success:
                            ...
                            "reasons": [
                                {
                                    ...
                                    "entity": (r.get("section") or r.get("entity_name") or "") if isinstance(r, dict) else getattr(r, "entity_name", ""),
                                    "file": r.get("file", "") if isinstance(r, dict) else "",
                                    "reason": (r.get("reason") or "") if isinstance(r, dict) else getattr(r, "reason", ""),
                ...
                if result.drift_analysis and result.drift_analysis.required_updates and workflow_config:
                    ...
                    class DriftRecordProxy:
                        def __init__(self, owner, repo, pr_number, head_sha,
                                     drift_score, severity, summary, entities):
                            self.owner = owner
                            self.repo = repo
...
async def process_fix_pr(job_id: str, auto_merge: bool = False) -> None:
    ...
    from src.pipeline.job_manager import get_db, JobStatus, SessionLocal
    ...
    if not tenant or not tenant.githubOrgId:
        ...
        job_manager.fail_job(job_id, "Tenant configuration incomplete (missing GitHub Installation)")
        return
    ...
    try:
        ...
        if _existing_fix_pr_url:
            ...
            with SessionLocal() as db:
                ...
                if _job:
                    ...
                    db.commit()
            job_manager.complete_job(job_id, {**(job.result or {}), "fixPrUrl": _existing_fix_pr_url})
            return
        ...
        if branch_name:
             ...
             _statuses = [u.get("recheck_status") for u in updates_raw if isinstance(u, dict) and u.get("recheck_status")]
             if not _statuses:
                 ...
                 _agg_recheck = "failed"
             _confidences = [u.get("recheck_confidence") for u in updates_raw if isinstance(u, dict) and u.get("recheck_confidence") is not None]
                 ...
                 if skip_reason is None:
                     ...
                     if _gh_issue_num and _gh_issue_repo and tenant.workflowConfig:
                         try:
                             ...
                             logger.info("EPIC-05: GitHub issue closed after auto-merge", issue=_gh_issue_num)
                         except Exception as _e:
             ...
             if jira_ticket_key and tenant.workflowConfig:
                 try:
                     ...
                     logger.warning("Failed to post Jira fix PR comment", ticket=jira_ticket_key, error=str(e))
        else:
